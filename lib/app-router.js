...
Router.map(function(){
    ...
    Router.route('upload', {
        ...
        action: function(req, res, next){
            // Here we need to do couple of things
            // 1. Save the file to a local directory - done
            //
            // 2. Save the filename to collection (of images) and to the UserProfile object for that user.
            // Done - the mechanism to save all details about the product to the UserProfile is done.
            // Do we need to save it in additional collection called, lets say 'Products' as well ???
            //
            // 3. Upload the image to a cloud or wherever it will be stored ultimately.
            ...
            // 1. Save to /public/uploads/images/ for later use
            // TODO This needs a mechanism to periodically clear old files or it will grow exponentially
            // TODO Find a way to save new uploaded files without reloading the app
            // Probably need implementation with the FS npm package
            // Update - it will ultimately use the image service
            // Alternatively we can skip the saving on the server local dir and just save in the cloud for scalability
            fs.createReadStream(req.filenames[0]).pipe(fs.createWriteStream(baseUrl + '/private/.userfiles/'+ userId +'-'+ filenameOnly));

            // 2. Save the filename to collection and to the UserProfile object

            res.write(baseUrl + '/private/.userfiles/'+ userId +'-'+ filenameOnly);
            res.end();
