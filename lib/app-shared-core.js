        ...
        Meteor.call('getEpsCount', function(err, res){
            ...
            if(err){
                ...
                if(Session.get('EpsCount') > 0){
                    ...
                    Session.set('paginationPages', Math.ceil(Session.get('EpsCount') / Session.get('paginationItemsPerPage')));
            ...
            init: function() {
                ...
                this.on("complete", function(file) {
                    ...
                    Meteor.call('readJSONFile', function(err, res){
                        if(err){
                           Bert.alert('An error occurred: '+ err.reason, 'danger');

                        } else {
                            Bert.alert('File processed successfully! Created: '+ res.created +' Already existing: '+ res.existing +' Invalid: '+ res.invalid, 'success');
                        }
                    });
                });
            }
